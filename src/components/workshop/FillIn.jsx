...
export default function FillIn({ onScore }) {
  ...
  const check = () => {
    setChecked(true)
    const correct = fillInItems.filter((item, i) => answers[i] === item.correct).length
    onScore({ correct, total: fillInItems.length, answers })
  }

  return (
    <div>
      <p style={{ color: '#4A4A4A', fontSize: '0.87rem', marginBottom: '1.2rem' }}>
        Choose the correct verb form to complete each sentence. Pay attention to tense and verb type.
      </p>

      {fillInItems.map((item, idx) => {
        ...
        return (
          <div key={idx} style={{ background: '#fff', border: '1px solid #EDE8DC', borderRadius: 10, padding: '1.4rem', marginBottom: '1rem', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
            ...
            {checked && (
              <div className={`feedback ${isCorrect ? 'feedback-ok' : 'feedback-err'}`} style={{ marginTop: '0.7rem', fontSize: '0.83rem' }}>
                {isCorrect ? '✓ Correct! ' : `✗ Correct answer: ${item.opts[item.correct]}. `}💡 {item.hint}
              </div>
            )}
          </div>
        )
      ...
      {checked && (
        <div style={{ background: 'rgba(26,39,68,0.06)', borderRadius: 10, padding: '1rem 1.4rem', marginTop: '0.5rem', fontWeight: 600, color: '#1A2744' }}>
          Score: {fillInItems.filter((item, i) => answers[i] === item.correct).length} / {fillInItems.length}
        </div>
      )}
    </div>
  )
